...
public class LogicalOrOfTwoBinaryGridsRepresentedAsQuadTrees {
    ...
    public Node intersect(Node quadTree1, Node quadTree2) {
        ...
        //下面判断子节点是否需要合并，也就是说是否需要生成新的子节点
        boolean a = ans.topLeft.isLeaf && ans.topRight.isLeaf && ans.bottomRight.isLeaf && ans.bottomLeft.isLeaf;//是否四个子节点都是叶子
        boolean b = ans.topLeft.val && ans.topRight.val && ans.bottomLeft.val && ans.bottomRight.val;//同时是否都是true
        boolean c = ans.topLeft.val || ans.topRight.val || ans.bottomLeft.val || ans.bottomRight.val;//是否都是false
        boolean isLeaf = a && (b || !c);//true则是该节点为叶子
        ans.isLeaf = isLeaf;
...
class Node {
    public boolean val;//储存叶子结点所代表的区域的值。1 对应 True，0 对应 False；
    public boolean isLeaf;//当这个节点是一个叶子结点时为 True，如果它有 4 个子节点则为 False 。
    public Node topLeft;
    public Node topRight;
    public Node bottomLeft;
    public Node bottomRight;
