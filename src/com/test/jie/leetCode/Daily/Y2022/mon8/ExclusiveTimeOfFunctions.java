...
public class ExclusiveTimeOfFunctions {
    public static void main(String[] args) {
        ...
        e.exclusiveTime(2, Arrays.stream(new String[]{"0:start:0","1:start:2","1:end:5","0:end:6"}).collect(Collectors.toList()));
    }
    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log : logs) {
            ...
            int ts = Integer.parseInt(sp[2]);//timeStamp
            if (sp[1].equals("start")) {
                if (!d.isEmpty()) {
                    ans[d.peekLast()] += ts - lastTime;
                }
                lastTime = ts;
                d.addLast(idx);
            } else {
                ans[idx] += ts - lastTime + 1;
                d.pollLast();
                lastTime = ts + 1;
            }
        }
